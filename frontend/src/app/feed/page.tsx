      ...
      <div className="max-w-6xl mx-auto pt-14 flex gap-6 px-4 py-6">
        ...
        <aside className="hidden xl:block w-64 shrink-0 space-y-4 pt-1">
          <div className="card p-4">
            ...
            {['ai', 'nextjs', 'placement', 'opensource', 'ml', 'devops', 'react', 'python'].map((tag) => (
              <a key={tag} href={`/feed?tag=${tag}`} className="block py-1.5 text-sm text-sky-600 dark:text-sky-400 hover:text-sky-700 transition">
                #{tag}
              </a>
            ))}
          </div>
