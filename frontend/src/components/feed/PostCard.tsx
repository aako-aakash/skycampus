...
export default function PostCard({ post, onDelete }: Props) {
  ...
  const handleComment = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      await postAPI.createComment(post.id, { content: comment })
      setComment('')
    ...
    <div className="card p-5">
        ...
        <Link href={`/profile/${post.user.id}`} className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-sky-400 to-indigo-500 flex items-center justify-center text-white font-bold text-sm overflow-hidden shrink-0">
            {post.user.profilePicture
              ? <img src={post.user.profilePicture} alt={post.user.name} className="w-full h-full object-cover rounded-full" />
              : post.user.name[0]?.toUpperCase()}
          </div>
          <div>
            <div className="font-semibold text-sm text-slate-800 dark:text-slate-200">{post.user.name}</div>
      ...
      {post.tags?.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-3">
          {post.tags.map((t) => (
            <Link key={t} href={`/feed?tag=${t}`} className="text-xs text-sky-600 dark:text-sky-400 hover:text-sky-700 font-medium">#{t}</Link>
          ))}
        </div>
      )}
