...
const POST_SELECT = (viewerId?: string) => ({
  id: true, content: true, imageUrl: true, tags: true,
  likesCount: true, commentsCount: true, visibility: true, createdAt: true,
...
export const getFeedPosts = async (userId: string, cursor?: string, limit = 10) => {
  ...
  const ids = [userId, ...following.map((f: any) => f.followingId)]
  const posts = await prisma.post.findMany({
    where: { userId: { in: ids }, visibility: 'PUBLIC' },
    select: POST_SELECT(userId) as any,
  ...
  return {
    posts: data.map(({ likes, ...p }: any) => ({ ...p, isLiked: likes?.length > 0 })),
    nextCursor, hasMore,
  }
}
...
export const getUserPosts = async (targetId: string, viewerId: string, cursor?: string, limit = 10) => {
  const posts = await prisma.post.findMany({
    ...
    select: POST_SELECT(viewerId) as any,
    orderBy: { createdAt: 'desc' },
    take: limit + 1,
    ...(cursor ? { cursor: { id: cursor }, skip: 1 } : {}),
  })
...
export const createPost = (userId: string, data: {
  content: string
  imageUrl?: string
  tags?: string[]
  visibility?: string
