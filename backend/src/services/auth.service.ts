...
export const register = async (data: RegisterInput) => {
  ...
  if (exists) throw new AppError(exists.email === data.email ? 'Email already registered' : 'Username taken', 409)
  const password = await bcrypt.hash(data.password, 12)
  const { password:_, refreshToken:__, ...user } = await prisma.user.create({ data:{...data,password} })
  return user
}

export const login = async (data: LoginInput, res: Response) => {
  const user = await prisma.user.findUnique({ where:{email:data.email} })
  if (!user || !(await bcrypt.compare(data.password, user.password))) throw new AppError('Invalid credentials', 401)
...
export const getMe = async (userId: string) => {
  const user = await prisma.user.findUnique({ where:{id:userId}, select:{id:true,name:true,email:true,username:true,bio:true,university:true,branch:true,year:true,skills:true,interests:true,profilePicture:true,coverImage:true,socialLinks:true,createdAt:true,_count:{select:{followers:true,following:true,posts:true}}} })
